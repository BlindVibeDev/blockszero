...
import { generateAIResponse, generateAIResponseWithWebSearch, StreamEvent } from '../services/groq';
  ...
  try {
    ...
    if (preferredService === 'perplexity') {
      ...
      const response = useWebSearch
        ? await generateAIResponseWithWebSearch(message, handleStreamEvent, previousResponseId)
